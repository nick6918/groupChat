...
UPYun.prototype.getFileInfo = function(file, callback) {
  httpAction('HEAD', file, null, null, function(err, res) {
    if (!err) {
      if (callback) callback(err, {'type': res.headers['x-upyun-file-type'], 'size': res.headers['x-upyun-file-size'], 'date': res.headers['x-upyun-file-date']});
    } else {
...
function sign(method, uri, date, length) {
  var signd = method + '&' + uri + '&' + date + '&' + length +
    '&' + _password;
  return 'UpYun ' + _username + ':' + md5(signd);
}
    ...
    res.on('end', function() {
      ...
      if (outputFile) {
        ...
        fs.writeFile(outputFile, resData, 'utf8', function(err) {
          if (callback) callback(err, {headers: res.headers, body: resData});
        });
      } else {
        if (res.statusCode >= 400 && res.statusCode < 600) {
          if (callback) callback({statusCode: res.statusCode, message: resData});
        } else {
          callback(null, {
            headers: res.headers,
            body: resData
